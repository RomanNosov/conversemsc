...
class shopOrderSendToAxiomusController extends waJsonController
{
    ...
    public function send($order)
    {
		...
		$failprice = "failprice=\"100\" ";
	    }

            for ($i = 0; $i < $item["quantity"]; $i++) {
                $items .= "<item name=\"".str_replace("\"", "\\\"", $item['name'])."\" weight=\"0.500\" quantity=\"1\" price=\"$price\" $failprice/>";
                $itemsCount++;
            }
        ...
        $ukey = $deliveryType != 'post' && $deliveryType != 'postp' && $deliveryType != 'regcour' && $deliveryType != 'regpick' 
            ? '3ef50dc2084ad29b4cb1350f9203aee0' : '2c096b8ab5d6fd8bcf90c36348ea5b6b';
		...
		if (3 <= date('d') * 1 && date('d') * 1 <= 4 && date('m') * 1 == 11) {
            $ddate = date("Y")."-11-05";
        }
	...
	$email = " email=\"".$order["contact"]["email"]."\"";

        if ($deliveryType == 'regcour' || $deliveryType == 'regpick') {
            $checksum = md5('7998'.'u'.$itemsCount.$itemsCount);
        } else {
            $checksum = md5(($deliveryType == 'post' || $deliveryType == 'postp' ?  '7998' : '7997').'u'.$itemsCount.$itemsCount);
        }
        ...
        if ($deliveryType == 'regpick') {
            $xdoc .= "<order$email inner_id=\"".$order["id"]."\" name=\"".$order["contact"]["name"]."\"$office $dateAttr=\"$ddate\"$edateAttr b_time=\"10:00\" e_time=\"18:00\" incl_deliv_sum=\"$shipping\">";
        } else {
            $xdoc .= "<order$email$sms inner_id=\"".$order["id"]."\" name=\"".$order["contact"]["name"]."\"$office address=\"$address\"$mkad $dateAttr=\"$ddate\"$edateAttr ".(("b_time=\"10:00\" e_time=\"".($deliveryType != "spb1" ? "18:00" : "15:00")."\""))." incl_deliv_sum=\"$shipping\"$cityAttr places=\"1\"$postType>";
        }
