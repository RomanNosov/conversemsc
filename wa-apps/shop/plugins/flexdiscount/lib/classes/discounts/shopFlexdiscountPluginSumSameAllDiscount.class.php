...
class shopFlexdiscountPluginSumSameAllDiscount extends shopFlexdiscountMask
{
    ...
    public function execute($params, $order, $categories, $contact, $apply)
    {
        ...
        if ($price > $param_v) {
            ...
            if ($params['discount_percentage']) {
                $discount += max(0.0, min(100.0, (float) $params['discount_percentage'])) * $total_price / 100.0;
            }
            ...
            if ($params['affiliate_percentage']) {
                $affiliate += max(0.0, (float) $params['affiliate_percentage']) * $total_price / 100.0;
            }
